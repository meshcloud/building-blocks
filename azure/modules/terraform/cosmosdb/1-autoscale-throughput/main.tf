...
resource "azurerm_cosmosdb_account" "example" {
  ...
  consistency_policy {
    ...
    max_interval_in_seconds = 300    #Required when consistency_level is set to BoundedStaleness
    max_staleness_prefix    = 100000 #Required when consistency_level is set to BoundedStaleness
  }
  depends_on = [
    azurerm_resource_group.cosmosdb_rg
  ]
}
